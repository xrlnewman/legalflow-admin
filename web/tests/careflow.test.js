...
test('LegalFlow has appointment, schedule and followup data', async()=>{const source=await readFile(new URL('../src/main.js',import.meta.url),'utf8'); assert.match(source,/今日案件队列/); assert.match(source,/律师排班/); assert.match(source,/合规任务/); assert.match(source,/AP-0716-082/)})
